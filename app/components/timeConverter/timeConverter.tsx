...
export default function TimeConverter({ missionStart, leapSeconds }: { missionStart?: string, leapSeconds: Date[] }) {
    ...
    function gps_to_tai(gpsSeconds: number) {
        return gpsSeconds + GPS0;
    }

    // converts a UTC time to TAI time, both given in milliseconds since Unix epoch
    ...
    const radioClasses = "group flex size-5 rounded-full border bg-white data-checked:bg-blue-400";

    return (
        <div className="flex flex-col gap-4">
            <div className='flex flex-row gap-4 items-center'>
                <label>UTC:</label>
                <TimeInput seconds={tai_to_utc(tai)} t0={utcEpoch} enabled={true} missionStart={missionStart} format={timeFormat} />
            </div>
            <div className='flex flex-row gap-4 items-center'>
                <label>GPS:</label>
                <TimeInput seconds={tai_to_gps(tai)} t0={gpsEpoch} enabled={true} missionStart={missionStart} format={timeFormat} />
            </div>
            <div className='flex flex-row gap-4 items-center'>
                <label>TAI:</label>
                <TimeInput seconds={tai} t0={taiEpoch} enabled={true} missionStart={missionStart} format={timeFormat} />
            </div>
            <div className='flex flex-row gap-4 items-center'>
                Time format:
                <RadioGroup value={timeFormat} onChange={setTimeFormat}>
                    <Field key={1} className="flex items-center gap-2">
                        <Radio value="date"
                            className="group flex size-5 rounded-full border bg-white data-checked:bg-blue-400"
                        >
                        </Radio>
                        <Label>Date</Label>
                    </Field>
                    <Field key={2} className="flex items-center gap-2">
                        <Radio value="doy" className={radioClasses}>
                        </Radio>
                        <Label>DOY</Label>
                    </Field>
            ...
            <div className="flex gap-10">
                <a className={buttonClasses} onClick={toggleTicking}>{keepTicking ? 'Stop ticking' : 'Start ticking'}</a>
                <a className={buttonClasses} onClick={setTimeToNow}>Now</a>
            </div>
            <br />
        </div>
    );
